...
def ERE(bamdir,strand):
    ...
    ERErun.write(' '.join(ERE_tools)+'\n')
    ERErun.close()
    os.chmod('ERE.sh',0o755)
    command = "sh ERE.sh > ERE.log"
    run_shell(command)
...
def DenoiseIntrons(maxintrons):
    ...
    Denois.write(' '.join(denoised)+'\n')
    Denois.close()
    os.chmod('DenoiseI.sh',0o755)
    command = "sh DenoiseI.sh >DenoiseI.log"
    run_shell(command)
...
def searchhit(ref):
    ...
    for name in os.listdir(currentdir+'/Extractor/'):
        ...
        mapped.write(' '.join(refdb)+'\n')
        mapped.write('echo "$?: make refDB"\n')
        mapped.write(' '.join(qrydb)+'\n')
        mapped.write('echo "$?: make queryDB"\n')
        #mapped.write(' '.join(createindex)+'\n')
        #mapped.write('echo "$?: create index"\n')
        mapped.write(' '.join(search)+'\n')
        mapped.write('echo "$?: search"\n')
        mapped.write(' '.join(convertalis)+'\n')
...
def GMMfilter():
    ...
    clean.write(' '.join(Filter)+' f="score/aa>=1.0"'+'\n')
    clean.write(' '.join(Filter)+' f="score/aa>=2.0"'+'\n')
    clean.write(' '.join(Filter)+' f="score/aa>=3.0"'+'\n')
    clean.write(' '.join(Filter)+' f="score/aa>=4.0"'+'\n')
    clean.write(' '.join(Filter)+' f="score/aa>=5.0"'+'\n')
...
def AnnotationFinalizer(ref,name):
    ...
    Final.write(' '.join(finalizes)+' a=filtered_predictions.gff outdir=Result_score2aa_1.0/'+'\n')
    Final.write(' '.join(finalizes)+' a=filtered_predictions_1.gff outdir=Result_score2aa_2.0/'+'\n')
    Final.write(' '.join(finalizes)+' a=filtered_predictions_2.gff outdir=Result_score2aa_3.0/'+'\n')
    Final.write(' '.join(finalizes)+' a=filtered_predictions_3.gff outdir=Result_score2aa_4.0/'+'\n')
    Final.write(' '.join(finalizes)+' a=filtered_predictions_4.gff outdir=Result_score2aa_5.0/'+'\n')
